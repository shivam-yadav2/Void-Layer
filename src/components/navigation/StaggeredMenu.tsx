...
export const StaggeredMenu: React.FC<StaggeredMenuProps> = ({
  ...
  useLayoutEffect(() => {
    const ctx = gsap.context(() => {
      ...
      if (!panel || !lineTop || !lineMid || !lineBottom || !icon) return;
      ...
      gsap.set(lineTop, { transformOrigin: "50% 50%", rotate: 0, y: -7 });
      gsap.set(lineMid, { transformOrigin: "50% 50%", rotate: 0, y: 0, opacity: 1 });
      gsap.set(lineBottom, { transformOrigin: "50% 50%", rotate: 0, y: 7 });
  ...
  const animateIcon = useCallback((opening: boolean) => {
    ...
    const lineMid = lineMidRef.current;
    const lineBottom = lineBottomRef.current;
    if (!icon || !lineTop || !lineMid || !lineBottom) return;
    ...
    if (opening) {
      ...
      spinTweenRef.current = gsap
        ...
        .to(lineTop, { y: 0, rotate: 45, duration: 0.5 }, 0)
        .to(lineBottom, { y: 0, rotate: -45, duration: 0.5 }, 0)
        .to(lineMid, { opacity: 0, duration: 0.2 }, 0.05);
    } else {
      spinTweenRef.current = gsap
        .timeline({ defaults: { ease: "power3.inOut" } })
        .to(lineTop, { y: -7, rotate: 0, duration: 0.35 }, 0)
        .to(lineBottom, { y: 7, rotate: 0, duration: 0.35 }, 0)
        .to(lineMid, { opacity: 1, duration: 0.2 }, 0.05)
    ...
    <div className={`sm-scope z-40 ${isFixed ? "fixed top-0 left-0 w-screen h-screen" : "w-full h-full"}`}>
      ...
      > 
        ...
        >
          ...
          <div className="sm-header-actions pointer-events-auto inline-flex items-center gap-3">
            {headerRightSlot}
            <button
              ref={toggleBtnRef}
            ...
            >
              ...
              <span ref={iconRef} className="sm-icon relative w-[24px] h-[16px] shrink-0 inline-flex items-center justify-center" aria-hidden="true">
                <span ref={lineTopRef} className="sm-icon-line sm-icon-line-top absolute left-1/2 top-1/2 w-full h-[2px] bg-current rounded-[2px]" />
                <span ref={lineMidRef} className="sm-icon-line sm-icon-line-mid absolute left-1/2 top-1/2 w-full h-[2px] bg-current rounded-[2px]" />
                <span ref={lineBottomRef} className="sm-icon-line sm-icon-line-bottom absolute left-1/2 top-1/2 w-full h-[2px] bg-current rounded-[2px]" />
              </span>
            </button>
          </div>
        </header>
        ...
        >
          <div className="sm-panel-inner flex-1 flex flex-col gap-5">
            ...
            <div className="sm-panel-bottom mt-auto pt-8 flex flex-col gap-5">
      ...
      <style>{`
.sm-scope { background: transparent; }
.sm-scope .staggered-menu-wrapper { position: relative; width: 100%; height: 100%; z-index: 40; pointer-events: none; background: transparent; }
.sm-scope .staggered-menu-header { position: absolute; top: 0; left: 0; width: 100%; display: flex; align-items: center; justify-content: space-between; padding: 2em; background: transparent; box-shadow: none; backdrop-filter: none; pointer-events: none; z-index: 20; }
.sm-scope .staggered-menu-header > * { pointer-events: auto; }
.sm-scope .sm-cta { transition: background 0.25s ease, color 0.25s ease, transform 0.25s ease; }
.sm-scope .staggered-menu-wrapper[data-open] .sm-cta { background: #000000; color: #ffffff; }
.sm-scope .sm-logo { display: flex; align-items: center; user-select: none; }
.sm-scope .sm-logo-img { display: block; height: 32px; width: auto; object-fit: contain; }
.sm-scope .sm-toggle { position: relative; display: inline-flex; align-items: center; gap: 0.3rem; padding: 6px; background: transparent; border: none; cursor: pointer; color: #e9e9ef; font-weight: 500; line-height: 1; overflow: visible; }
.sm-scope .sm-toggle:focus-visible { outline: 2px solid #ffffffaa; outline-offset: 4px; border-radius: 4px; }
.sm-scope .sm-toggle-textWrap { position: relative; margin-right: 0.5em; display: inline-block; height: 1em; overflow: hidden; white-space: nowrap; width: var(--sm-toggle-width, auto); min-width: var(--sm-toggle-width, auto); }
.sm-scope .sm-toggle-textInner { display: flex; flex-direction: column; line-height: 1; }
.sm-scope .sm-toggle-line { display: block; height: 1em; line-height: 1; }
.sm-scope .sm-icon { position: relative; width: 24px; height: 16px; flex: 0 0 24px; display: inline-flex; align-items: center; justify-content: center; }
...
.sm-scope .sm-panel-bottom { display: flex; flex-direction: column; gap: 1.25rem; }
...
.sm-scope .sm-panel-item { position: relative; color: #000; font-weight: 600; font-size: 4rem; cursor: pointer; line-height: 1; letter-spacing: -2px; text-transform: uppercase; transition: transform 0.25s ease, opacity 0.25s ease; display: inline-block; text-decoration: none; padding-right: 1.4em; }
.sm-scope .sm-panel-item:hover { transform: translateX(6px); font-weight: 700; }
